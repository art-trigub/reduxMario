...
import { deleteItem } from '../../../store/actions/libraries/statusClient'
import { getData, onSave, editItem } from '../../../store/actions/libraries/statusClient'
import { changeListBreadCrumbs } from '../../../store/actions/breadCrumbs'
import '../style.css'


function StatusClient({ getData, list, isLoading, deleteItem, onSave, editItem }) {
    ...
    useEffect(() => {
        document.title = t('statusClient')
        dispatch(changeListBreadCrumbs([`${t('libraries')}`, `${t('statusClient')}`]))
    }, [t])
        ...
        <>
            {
                isLoading
                    ...
                    :
                    <Paper component='div'>
                        {visibleModal && <Modal className="modal" setEditMode={setEditMode} editItem={editItem} editingItem={editingItem} editMode={editMode} onSave={onSave} visible={visibleModal} closeModal={closeModal}></Modal>}
                        <IBoxHeader>
                            ...
                            <Table stickyHeader aria-label="sticky table" size="small" className={classes.table}>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <IHeaderTableCell align="center">{t("monthlyFee")}</IHeaderTableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {(rowsPerPage > 0
...
function mapStateToProps({ statusClient }) {
    return {
        list: statusClient.list,
        isLoading: statusClient.isLoading
    }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(StatusClient);
